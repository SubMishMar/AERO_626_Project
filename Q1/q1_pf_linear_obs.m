function [ Xpred, sigma_xx, sigma_yy, sigma_tt, NEES, t ] = q1_pf_linear_obs(mu,sigma, xtrue, ztrue, t, g, Ts, rho_0, k_rho, Q, R, N)
...
for j = 1:length(t)
    ...
    for i = 1:N
        ...
        z_update(:,i) = linear_measurement_model(x_P_update(:,i));

        P_w(:,i) = (1/sqrt(det(2*pi*Q))) * exp(-(ztrue(:,j+1) - z_update(i))^2/(2*Q));
    end
    if length(find(P_w == zeros)) == length(P_w)
        break;
    end
