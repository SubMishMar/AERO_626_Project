function [ Xpred, sigma_xx, sigma_yy, sigma_tt, NEES, t ] = q1_enkf_nonlinear_obs(mu,sigma, xtrue, ztrue, t, g, T, rho_0, k_rho, Q, R, n_ensembles)
...
K = Pf*H'/(H*Pf*H'+Q);
for j = 1:n_ensembles
    X(:,j) = X(:,j) + K*(ztrue(:,i+1) - nonlinear_measurement_model(X(:,j)));
end
xa = mean(X,2);
Pa = cov(X');
sigma = Pa;
