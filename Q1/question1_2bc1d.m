...
Q_nonlinear = 200; %% Measurement Noise
Q_nonlinear_PF = 2 * 10^(12);%% Measurement Noise for PF
H = [1, 0, 0];
...
for count = 1:50
    ...
    % Filter
    [Xpred, sigma_xx, sigma_yy, sigma_tt, NEES_kf, time] = q1_ekf_nonlinear_obs(mu, sigma, xtrue, ztrue_nonlinear, t, g, T, rho_0, k_rho, Q_nonlinear, R);
    NEES_KF = NEES_KF + NEES_kf; 
    errors(:, :, count) = abs(Xpred - xtrue);
end
mean_error_ekf = mean(mean(errors, 2),3);
timervalue_ekf = toc/count;
NEES_KF = NEES_KF/count;
...
for count = 1:50
    ...
    % Filter
    [Xpred, sigma_xx, sigma_yy, sigma_tt, NEES_ukf, time] = q1_ukf_nonlinear_obs( mu,sigma, xtrue, ztrue_nonlinear, t, g, T, rho_0, k_rho, Q_nonlinear, R);
...
for count = 1:50
    ...
    % Filters
    [ Xpred, sigma_xx, sigma_yy, sigma_tt, NEES_enkf, time ] = q1_enkf_nonlinear_obs(mu,sigma, xtrue, ztrue_nonlinear, t, g, T, rho_0, k_rho, Q_nonlinear, R, n_ensembles);
...
for count = 1:50
    ...
    % Filters
    [ Xpred, sigma_xx, sigma_yy, sigma_tt, NEES_pf, time ] = q1_pf_nonlinear_obs(mu,sigma, xtrue, ztrue_nonlinear, t, g, T, rho_0, k_rho, Q_nonlinear_PF, R, N);
...
box off
title('Execution time for 1 loop in seconds for 1: EKF, 2: UKF, 3: EnKF, 4: PF');
